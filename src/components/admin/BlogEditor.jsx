...
import {
  ...
  listContainer, scrollList, miniHeader,
  listItem, deleteButton, editButton, iconDeleteBtn,
  gridThumbs, thumbBox, thumbImg, thumbClose,
...
export default function BlogEditor({ blogs, onBlogsChange }) {
  ...
  const onVideoDragStart = (i) => setDragBlogIdx(i)
  const onVideoDragOver = (e) => e.preventDefault()
  const onVideoDrop = (i) => {
    if (dragBlogIdx === null || dragBlogIdx === i) return
    setBlogForm(prev => ({ ...prev, videos: moveItem(prev.videos || [], dragBlogIdx, i) }))
    setDragBlogIdx(null)
  }
    ...
    <div style={sectionCard}>
      ...
      <form onSubmit={handleBlogSubmit} style={formLayout}>
        ...
        <input
          ...
          onChange={e => setBlogForm(prev => ({ ...prev, video_url: e.target.value }))}
          style={inputField}
        />
        ...
        <div style={{ ...builderBox, padding: "1rem", marginTop: "1rem" }}>
          ...
          <FileDropMultiVideo
            ...
            folder="videos"
            onUploaded={(urls) => setBlogForm(prev => ({ ...prev, videos: [...(prev.videos || []), ...urls] }))}
          />
