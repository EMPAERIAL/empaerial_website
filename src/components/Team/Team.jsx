...
function hasDetails(member) {
  return Boolean(member?.role || member?.skills || member?.funFact);
}
...
export default function Team({ t }) {
  ...
  // Touch devices expand details in flow instead of hovering a tooltip.
  ...
  const positionTooltip = useCallback(
    (index) => (event) => {
      ...
      const projectedTop =
        card.getBoundingClientRect().top - 12 - tooltip.offsetHeight;
      setFlippedCard(
        projectedTop < grid.getBoundingClientRect().top ? index : null
      );
    },
    []
  );
      ...
      <div className="sec-inner">
        ...
        <div className="sec-head reveal">
          ...
          <div className={styles.titleRow}>
            <h2 id="team-title" className={`sec-h2 ${styles.headTitle}`}>
              {t.team_title}
            </h2>
        ...
        {/* Tab description */}
        <p className={styles.tabDesc}>{t[TAB_MAP[activeKey].descKey]}</p>
        ...
        {loading ? (
          ...
          <p className={styles.status} role="status">
            {t.team_error ||
              "We could not load the roster right now. Please try again shortly."}
          </p>
        ) : members.length === 0 ? (
          <p className={styles.status} role="status">
          ...
          >
            {members.map((member, i) => (
              ...
              >
                ...
                {/*
                  Touch devices get a real control: the tooltip is hover-only,
                  so on a phone the role, skills and fun fact were unreachable.
                  Hidden from pointer devices, where hovering already works.
                */}
                {hasDetails(member) && (
                  <button
                    type="button"
                    ...
                    aria-controls={`member-details-${i}`}
                    onClick={() =>
                      setExpandedCard((current) => (current === i ? null : i))
                    }
                  >
                    {expandedCard === i
                      ? t.team_hide_details || "Hide details"
                      : t.team_show_details || "Details"}
                  </button>
                )}

                {/* Hover tooltip (pointer devices) / expanded panel (touch) */}
                <div
